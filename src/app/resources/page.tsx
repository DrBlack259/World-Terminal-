...
import { Pickaxe, MapPin, Building2, Calendar } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-5 gap-2 p-3 border-b border-terminal-border flex-shrink-0">
        ...
        <StatCard label={`Est. Total Value (${selectedCurrency})`} value={fmt(totalValueMn)} color="amber" />
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto p-3 grid grid-cols-2 gap-2 content-start">
          {filtered.map((r) => (
            <div key={r.id} onClick={() => setSelected(r)} className={`terminal-panel border p-3 cursor-pointer hover:border-terminal-green/40 transition-all ${selected?.id === r.id ? "border-terminal-green/60 bg-terminal-green/5" : "border-terminal-border"}`}>
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="text-right">
                  {/* estimatedValueBn (USD billions) → convert: *1000 to get USD millions */}
