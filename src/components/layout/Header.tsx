...
import { AlertTriangle, Wifi, Clock, Shield, Zap, ChevronDown, RefreshCw, CheckCircle, XCircle, BarChart2, Menu } from "lucide-react";
...
export default function Header({ onMenuToggle }: HeaderProps) {
  ...
  return (
<header className="h-12 flex-shrink-0 bg-terminal-panel terminal-surface border-b border-terminal-border/60 border-x-0 border-t-0 rounded-none flex items-center px-2 sm:px-4 gap-2 sm:gap-4 relative z-50 shadow-[0_10px_30px_rgba(2,7,18,0.4)]">
      ...
      <div className="flex items-center gap-1.5 mr-1">
        <AlertTriangle className="w-3.5 h-3.5 text-terminal-red animate-pulse flex-shrink-0" />
        <span className="text-terminal-red text-[10px] font-bold tracking-widest uppercase hidden sm:inline">
          ALERT: {globalStats.alertLevel}
        </span>
        <span className="text-terminal-red text-[10px] font-bold sm:hidden">
          {globalStats.alertLevel}
        </span>
      </div>

      {/* Stats — hidden on very small screens */}
      <div className="hidden md:flex items-center gap-4 text-[10px] text-terminal-text-dim">
      ...
      <div className="ml-auto flex items-center gap-2 sm:gap-3">
        ...
        >
          ...
          <span className="hidden sm:inline">{isVisual ? "← TERMINAL" : "VISUAL"}</span>
        </Link>
...
className="flex items-center gap-1 sm:gap-1.5 bg-terminal-bg/80 border border-terminal-border rounded-lg px-2 py-1.5 text-[10px] hover:border-terminal-green/50 transition-colors"
          >
            ...
            <span className="text-terminal-text-dim hidden sm:inline">{info.symbol}</span>
            {rateVsUSD && <span className="text-terminal-text-dim hidden lg:inline">{rateVsUSD}</span>}
            ...
            {!ratesError && !ratesLoading && lastUpdated && <CheckCircle className="w-2.5 h-2.5 text-terminal-green/60 hidden sm:inline" />}
            ...
            <div className="absolute right-0 top-full mt-2 w-72 bg-terminal-panel border border-terminal-border rounded-xl shadow-2xl z-50 flex flex-col max-h-80 sm:max-h-96">
              <div className="px-3 py-2 border-b border-terminal-border flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  {ratesError ? (
                    <span className="text-[8px] text-terminal-amber flex items-center gap-1"><XCircle className="w-2.5 h-2.5" /> Fallback</span>
                  ) : lastUpdated ? (
                  ...
                  <button onClick={(e) => { e.stopPropagation(); refreshRates(); }} className="text-terminal-text-dim hover:text-terminal-green transition-colors" title="Refresh rates">
                ...
                {filtered.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`font-bold text-[10px] w-8 flex-shrink-0 ${isSelected ? "text-terminal-green" : "text-terminal-text"}`}>{c.code}</span>
                      ...
                      <span className={`text-[9px] font-mono flex-shrink-0 ${isSelected ? "text-terminal-green" : "text-terminal-text-dim"}`}>
                        {c.code === "USD" ? "1.0000" : liveRate ? liveRate.toFixed(["JPY","KRW","IDR","NGN","PKR","TRY","RUB","INR"].includes(c.code) ? 2 : c.code === "KWD" ? 4 : 4) : "—"}
                      </span>
                    </button>
                  );
                })}
                {filtered.length === 0 && <div className="px-3 py-4 text-center text-terminal-text-dim text-[9px]">No currency found</div>}
              </div>
        ...
        {/* Right indicators */}
        <div className="hidden sm:flex items-center gap-1 text-[10px] text-terminal-text-dim">
          <Zap className="w-3 h-3 text-terminal-green" />
          <span className="text-terminal-green hidden lg:inline">AI ONLINE</span>
        </div>
        <div className="hidden md:flex items-center gap-1 text-[10px] text-terminal-text-dim">
          <Wifi className="w-3 h-3 text-terminal-green" />
          <span className="hidden lg:inline">FEED OK</span>
        </div>
        <div className="hidden sm:flex items-center gap-1 text-[10px] text-terminal-text-dim">
          <Clock className="w-3 h-3" />
          <span className="font-mono text-terminal-cyan">{localStr}</span>
          <span className="text-terminal-text-muted ml-1 hidden lg:inline">UTC</span>
          <span className="font-mono text-terminal-text hidden lg:inline">{utcH}:{utcM}:{utcS}</span>
        </div>
      </div>
    </header>
  );
